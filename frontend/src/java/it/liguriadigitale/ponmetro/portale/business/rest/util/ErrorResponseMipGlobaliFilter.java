...
public class ErrorResponseMipGlobaliFilter implements ClientResponseFilter {
  ...
  public void filter(ClientRequestContext requestContext, ClientResponseContext responseContext)
      throws IOException {
    log.debug("CP filter mip globali " + responseContext.getStatus());

    if (responseContext.getStatus() == Response.Status.NOT_FOUND.getStatusCode()) {

      log.debug(
          "[ErrorResponseMipGlobaliFilter] invocazione di:"
      ...
      log.debug("[ErrorResponseMipGlobaliFilter] Errore Mip Globali:" + message);
      // throw new WebApplicationException(message);
      // throw new RestartResponseAtInterceptPageException(new
      // MipGlobaliErrorPage(message));
    }
  }
}

package it.liguriadigitale.ponmetro.portale.presentation.pages.domandeseparazionidivorzi.richiesta;
...
public class RichiestaSeparazioneDivorzioPage extends LayoutBasePage {

  public RichiestaSeparazioneDivorzioPage() {
    super();

    if (Constants.DEPLOY == Constants.TIPO_DEPLOY.ESERCIZIO) {
      throw new RestartResponseAtInterceptPageException(
          new RedirectPage(BaseServiceImpl.FDC2_RICHIESTA_DOMANDA_SEPARAZIONE_DIVORZI));
    }

    addOrReplace(
        new IntegrazioneFdCClassicFdC2Panel(
            "integrazionePanel", BaseServiceImpl.FDC2_RICHIESTA_DOMANDA_SEPARAZIONE_DIVORZI));
  }
}

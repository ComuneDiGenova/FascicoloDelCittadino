...
public class LinkDinamicoHomeRicercaGlobo<T> extends Panel {
  ...
  public LinkDinamicoHomeRicercaGlobo(
  ...
  public LinkDinamicoHomeRicercaGlobo(String id, LinkDinamicoFunzioneData linkF, T t) {
    this(id, linkF, t, null);
  }

  public LinkDinamicoHomeRicercaGlobo(String id, LinkDinamicoFunzioneData linkF) {
    ...
    link =
        new Link<Void>(LINK_ID) {
          ...
          public void onClick() {
            try {
              log.debug(
                  ":: LinkDinamicoHomeRicercaGlobo > creaLink > onClick : cerco classe "
              ...
              log.debug(
                  ":: LinkDinamicoHomeRicercaGlobo > creaLink > onClick : class "
                      + linkF.getWicketClassName()
                      + " trovata ");
              if (t != null) {
              ...
              log.debug("LinkDinamicoHomeRicercaGlobo > object =\n" + object.toString());
              setResponsePage((IRequestablePage) object);

              log.debug("end > onClick > LinkDinamicoHomeRicercaGlobo");

            } catch (RestartResponseAtInterceptPageException e) {
              throw e;
            } catch (InstantiationException e) {
              log.error(
                  "[LinkDinamicoFunzioneRicercaGlobo1] onClick error on reflection: "
                      + e.getMessage(),
                  e);
            } catch (IllegalAccessException e) {
              log.error(
                  "[LinkDinamicoFunzioneRicercaGlobo2] onClick error on reflection: "
                      + e.getMessage(),
                  e);
            } catch (IllegalArgumentException e) {
              log.error(
                  "[LinkDinamicoFunzioneRicercaGlobo3] onClick error on reflection: "
                      + e.getMessage(),
                  e);
              ...
              log.error(
                  "[LinkDinamicoFunzioneRicercaGlobo4] onClick error on reflection: "
                      + e1.getMessage(),
                  e);
              throw new RestartResponseException(new MipGlobaliErrorPage(e1.getMessage()));
            } catch (NoSuchMethodException e) {
              log.error(
                  "[LinkDinamicoFunzioneRicercaGlobo5] onClick error on reflection: "
                      + e.getMessage(),
                  e);
            } catch (SecurityException e) {
              log.error(
                  "[LinkDinamicoFunzioneRicercaGlobo6] onClick error on reflection: "
                      + e.getMessage(),
                  e);
            }
          }

...
import it.liguriadigitale.ponmetro.portale.presentation.components.select2.json.JsonBuilder;
...
public class Select2MultiChoice<T> extends AbstractSelect2Choice<T, Collection<T>> {
  ...
  public void convertInput() {
    ...
    String input =
        getWebRequest().getRequestParameters().getParameterValue(getInputName()).toString();
  ...
  protected String getModelValue() {
    ...
    // if values is null or empty set value attribute to an empty string rather then '[]' which does
    // not make sense

...
public class Step5IstanzaPlConfermaDatiIstanzaForm
    extends AbstracFrameworkForm<DatiRichiestaIstanzaPl> {

  private static final long serialVersionUID = -7349674609304452360L;

  public Step5IstanzaPlConfermaDatiIstanzaForm(String id, DatiRichiestaIstanzaPl model) {
    super(id, model);
  ...
  public void addElementiForm() {
    ...
    residenzaNumeroCivico.setVisible(false);
    ...
    String strDescrizioneTargaErrata =
        "Il veicolo " + veicolo + " non è di mia proprietà, sono  proprietario di una ";

    if (getModelObject().getMarca() != null) {
      strDescrizioneTargaErrata = strDescrizioneTargaErrata + " " + getModelObject().getMarca();
    }

    if (getModelObject().getModello() != null) {
      strDescrizioneTargaErrata = strDescrizioneTargaErrata + " " + getModelObject().getModello();
    }

    TextField descrizioneTargaErrata =
        new TextField("descrizioneTargaErrata", Model.of(strDescrizioneTargaErrata));
    descrizioneTargaErrata.setEnabled(false);
    descrizioneTargaErrata.setVisible(!getModelObject().getMiaMacchina().equalsIgnoreCase("Sì"));
    addOrReplace(descrizioneTargaErrata);

    String strDescrizioneNonProrietario = "Ia " + veicolo + " non è di mia proprietà.";

    if (getModelObject().getDataInizioProprieta() != null) {
      strDescrizioneNonProrietario =
          strDescrizioneNonProrietario
              + " La mia proprietà è iniziata il "
              + DateUtil.toStringFromLocalDate(getModelObject().getDataInizioProprieta());
    }

    if (getModelObject().getDataFineProprieta() != null) {
      strDescrizioneNonProrietario =
          strDescrizioneNonProrietario
              + " è terminata il "
              + DateUtil.toStringFromLocalDate(getModelObject().getDataFineProprieta());
    }

    TextField descrizioneNonPrprietario =
        new TextField("descrizioneNonPrprietario", Model.of(strDescrizioneNonProrietario));
    descrizioneNonPrprietario.setEnabled(false);
    descrizioneNonPrprietario.setVisible(getModelObject().getMaiMia().equalsIgnoreCase("NO"));
    addOrReplace(descrizioneNonPrprietario);
  }
}

...
public abstract class Select2ChoiceBaseComponent<M> extends HiddenField<M> {
  ...
  /**
   * Gets the markup id that is safe to use in jQuery by escaping dots in the default {@link
   * #getMarkup()}
   *
   * @return markup id
   */
  ...
  /**
   * Escapes single quotes in localized strings to be used as JavaScript strings enclosed in single
   * quotes
   *
  ...
  protected void onInitialize() {
    ...
    ajax.setData(
        String.format(
            "function(term, page) { return { term: term, page:page, '%s':true, "
                + "'%s':[window.location.protocol, '//', window.location.host, window.location.pathname].join('')}; }",
    ...
    getSettings()
        .setFormatNoMatches("function() { return '" + getEscapedJsString("noMatches") + "';}");
    getSettings()
    ...
    getSettings()
        .setFormatSelectionTooBig(
            "function(limit) { return limit == 1 ? '"
                ...
                + "'.replace('{limit}', limit); }");
    getSettings()
        .setFormatLoadMore("function() { return '" + getEscapedJsString("loadMore") + "';}");
    getSettings()
        .setFormatSearching("function() { return '" + getEscapedJsString("searching") + "';}");
  }
  ...
  protected void onRemove() {
    ...
    if (target != null) {
      ...
      target.prependJavaScript(
          String.format("$('#%s').select2('close');", getJquerySafeMarkupId()));
    }
  }
}

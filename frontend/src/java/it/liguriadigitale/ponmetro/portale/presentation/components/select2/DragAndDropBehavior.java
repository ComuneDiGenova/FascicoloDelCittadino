...
/**
 * Adds drag and drop behavior to Select2MultiChoice components, i.e. the list of selected values
 * can be sorted.
 *
...
public class DragAndDropBehavior extends Behavior {
  ...
  public void renderHead(Component component, IHeaderResponse response) {
    ...
    String script =
        "$('#%1$s').select2('container').find('ul.select2-choices').sortable({"
            + "containment: 'parent',"
            + "start: function() { $('#%1$s').select2('onSortStart'); },"
            + "update: function() { $('#%1$s').select2('onSortEnd'); }"
            + "});";
    response.render(
        OnDomReadyHeaderItem.forScript(JQuery.execute(script, component.getMarkupId())));
  }
}

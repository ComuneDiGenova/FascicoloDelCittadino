...
public class ListItemCodiceFDCRender implements IChoiceRenderer<ListItem> {
  ...
  public String getIdValue(ListItem obj, final int index) {
    ListItem item = obj;
    if (LabelFdCUtil.checkIfNotNull(item) && LabelFdCUtil.checkIfNotNull(item.getCodiceFDC())) {
      return String.valueOf(item.getCodiceFDC());
    } else {
      return "";
    }
  }
  ...
  public ListItem getObject(String id, IModel<? extends List<? extends ListItem>> lista) {
    for (ListItem dati : lista.getObject()) {
      if (LabelFdCUtil.checkIfNotNull(dati))
        if (dati.getCodiceFDC().equalsIgnoreCase(id)) return dati;
    }

 ...
 *
 * <p>The two important services provided by implementations are:
 *
 * <ul>
 *   <li>Feeding choices, represented by the application's domain objects, to Select2 components
 *   <li>Transcoding said choices to and from Json so they can consumed by Select2
 * </ul>
 *
 * <p>For the most common usecase where each choice is rendered as a text string see {@link
 * TextChoiceProvider}.
 *
...
public abstract class ChoiceProvider<T> implements IDetachable {
  ...
  /**
   * Queries application for choices that match the search {@code term} and adds them to the {@code
   * response}
   *
   ...
   * <p>
   *
   * <p>At the very least each choice should contain an {@code id} attribute. If no custom rendering
   * function is specified, the {@code text} attribute should also be provided
   *
   * <p>
   *
   * <p>Example: If mapping a User {Long id, String name} using default rendering the code should
   * look like this:
   *
   * <p>
   *
  ...
  /**
   * Converts a list of choice ids back into application's choice objects. When the choice provider
   * is attached to a single-select component the {@code ids} collection will contain exactly one
   * id, and a collection containing exactly one choice should be returned.
   *
